  ...
  return (
    <header className="mb-6">
      <div className="flex flex-wrap items-center gap-3">
        <div className="text-[15px] font-extrabold tracking-tight" style={{ color: 'var(--ink)' }}>
      ...
      <p className="mt-1.5 text-[13px]" style={{ color: 'var(--muted)' }}>{data.mission}</p>
      <div className="mt-1 flex items-center gap-2 text-[12px]" style={{ color: 'var(--faint)' }}>
        ...
        <span
          ...
          style={{
            ...
            background: `color-mix(in srgb, ${stale ? 'var(--red)' : 'var(--green)'} 14%, transparent)`,
          }}
        >
...
function BlockerStrip({ data }: { data: BoardData }) {
  ...
  return (
    <div
      ...
      style={{ background: 'color-mix(in srgb, var(--red) 12%, var(--panel))', border: '1px solid color-mix(in srgb, var(--red) 40%, var(--line))' }}
    >
...
function ProjectCard({ p, onOpen }: { p: Project; onOpen: () => void }) {
  ...
  return (
    <button
      onClick={onOpen}
      className="accentbar lift panel relative w-full rounded-2xl p-5 text-left"
    ...
    >
      ...
      <p className="mt-1 text-[12.5px] leading-snug" style={{ color: 'var(--muted)' }}>{p.blurb}</p>

      <div className="mt-3 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <HealthChip health={p.health} />
          <span className="flex items-center gap-1 text-[11px]" style={{ color: 'var(--muted)' }}>
            <Delta value={p.momentum.commitsDelta} /> commits
          </span>
        </div>
        <Sparkline data={p.momentum.spark} color={accent} />
      </div>

      <div className="mt-1.5 text-[11px]" style={{ color: 'var(--faint)' }}>
        {p.version !== '—' ? `${p.version} · ` : ''}updated {p.updatedDays}d ago
      </div>
...
/* ── Detail view ── */
function Stat({ label, value, delta, accent }: { label: string; value: number; delta: number; accent: string }) {
  return (
    ...
    <div style={{ ['--accent' as any]: accent }}>
      ...
      <div className="panel accentbar relative rounded-2xl p-6">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <StatusPill status={p.status} />
          <HealthChip health={p.health} />
          {p.version !== '—' && <span className="text-[12px]" style={{ color: 'var(--faint)' }}>{p.version}</span>}
          <div className="ml-auto flex items-center gap-3">
            <Avatars owners={p.owners} size={28} />
            {p.links.map(l => (
              <a key={l.label} href={l.url} className="text-[12px]" style={{ color: 'var(--blue)' }}>{l.label} ↗</a>
            ))}
          </div>
        </div>
        <p className="mt-1.5 text-[13px]" style={{ color: 'var(--muted)' }}>{p.blurb}</p>

        {/* momentum */}
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Stat label="Commits (30d)" value={p.momentum.commits} delta={p.momentum.commitsDelta} accent={accent} />
          <Stat label="Merged PRs" value={p.momentum.prs} delta={p.momentum.prsDelta} accent={accent} />
          <Stat label="Open issues" value={p.momentum.issues} delta={p.momentum.issuesDelta} accent={accent} />
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-2">
            ...
            <ol className="relative">
              {p.shipped.map((s, i) => (
                <li key={i} className="relative pl-5 pb-4">
                  ...
                  <div className="text-[12.5px]" style={{ color: 'var(--ink)' }}>{s.summary}</div>
                  <div className="text-[11px] mt-0.5" style={{ color: 'var(--faint)' }}>
                    {s.version !== '—' ? `${s.version} · ` : ''}{s.date}
                  </div>
                </li>
              ))}
            </ol>
          ...
          <div>
            ...
            {p.inFlight.length === 0 && <div className="text-[12.5px]" style={{ color: 'var(--faint)' }}>Nothing open.</div>}
            {p.inFlight.map(pr => (
              <div key={pr.number} className="mb-2 rounded-lg px-3 py-2 text-[12.5px]" style={{ background: 'var(--panel2)', border: '1px solid var(--line-soft)' }}>
                <span style={{ color: 'var(--ink)' }}>{pr.title}</span>
                <span className="ml-1" style={{ color: 'var(--faint)' }}>#{pr.number} · {pr.ageDays}d</span>
              </div>
            ))}

            <div className="text-[10px] font-bold uppercase tracking-wider mt-5 mb-2" style={{ color: 'var(--faint)' }}>Next up</div>
            <div className="rounded-lg p-3" style={{ background: 'color-mix(in srgb, var(--accent) 10%, var(--panel2))', border: '1px solid color-mix(in srgb, var(--accent) 35%, var(--line))' }}>
              <span style={{ color: 'var(--ink)', fontSize: 13 }}>→ {p.next}</span>
            </div>
            {p.nextSuggested && (
              <div className="mt-2 text-[12px] dim-note italic">
                suggested: {p.nextSuggested}
              </div>
            )}
          </div>
        </div>
            ...
            {FILTERS.map(f => (
              <button
                ...
                style={{
                  ...
                  color: filter === f.key ? 'var(--bg1)' : 'var(--muted)',
